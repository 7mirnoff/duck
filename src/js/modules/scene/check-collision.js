...
const checkCollision = () => {
  ...
  for (let vertexIndex = 0; vertexIndex < APP.physDuck.geometry.vertices.length; vertexIndex++) {
    ...
    APP.collisionResults = ray.intersectObjects(APP.collidableMeshList)
    if (APP.collisionResults.length > 0 && APP.collisionResults[0].distance < directionVector.length()) {
      console.log('hit')
      return true
    } else {
